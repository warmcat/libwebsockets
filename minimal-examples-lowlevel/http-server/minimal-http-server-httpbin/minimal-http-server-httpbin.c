...
int main(int argc, const char **argv)
{
	 ...
	 * (which includes h3), creates a QUIC listener on the same port and
	 * advertises it with an alt-svc header... clients then race QUIC at
	 * later streams.  The h3 detour is not what these tests are about
	 * and behaves differently across tls backends and arches, so confine
