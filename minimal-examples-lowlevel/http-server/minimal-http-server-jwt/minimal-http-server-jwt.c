		...
		void *in, size_t len)
{
	...
	switch (reason) {
		...
		case LWS_CALLBACK_HTTP:
			...
			ck.cookie_name = "sai_jwt"; // Using simple name for example
			...
			if (!strcmp(url_path, "/logout")) {
				...
				n = lws_snprintf(cookie_clr, sizeof(cookie_clr),
						"sai_jwt=deleted; HttpOnly; SameSite=Strict; Path=/; expires=Thu, 01 Jan 1970 00:00:00 GMT");
			...
			if (!strcmp(url_path, "/api/secret")) {
				...
				if (lws_add_http_header_content_length(wsi, (lws_filepos_t)n, &p, end)) return 1;
				if (lws_finalize_write_http_header(wsi, start, &p, end)) return 1;
