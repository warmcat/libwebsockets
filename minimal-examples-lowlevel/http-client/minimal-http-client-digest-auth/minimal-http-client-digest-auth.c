...
static const char *ua = "Mozilla/5.0 (X11; Linux x86_64) "
		...
		"Chrome/51.0.2704.103 Safari/537.36", *acc = "*/*";
...
int auth_decode_digest_http_message(const char *chlg, struct digestdata *digest) {
	...
	while (1) {
		...
		if (0 == strncmp(sentence_start, "algorithm=", strlen("algorithm="))) {
			...
			if (sentence_end != NULL) {
				...
				digest->algorithm = malloc(
						sizeof(char) * strlen(sentence_start) + 1);
				if (NULL == digest->algorithm) {
					goto end_cleanup;
				}
				memset(digest->algorithm, 0x0, sentence_len + 1);
				strncpy(digest->algorithm, sentence_start,
						strlen(sentence_start));
				break;
			}
		}
	...
	return 0;
	end_cleanup: digest_cleanup(digest);
	end_error: return -1;
}
...
int output_digest(struct lws *wsi, char *http_req, char *uri, char *user,
		char *password, char *challenge, char **outptr, size_t *outlen) {
	...
	hashthis = malloc(
			sizeof(char)
					* (strlen(user) + strlen(password) + strlen(digest.realm)
							+ 3));
	if (!hashthis)
		return -1;
	...
	if (digest.qop && strcasecmp(digest.qop, "auth-int") == 0) {
		...
		hashthis2 = malloc(
				sizeof(char) * (strlen(hashthis) + strlen(hashed) + 2));
	...
	if (digest.qop) {
		hashthis = malloc(
				sizeof(char)
						* (strlen((char*) ha1) + 1 + strlen(digest.nonce) + 1
								+ 8 + 1 + strlen(digest.cnonce) + 1
								+ strlen(digest.qop) + 1) + strlen((char*) ha2)
						+ 1);
	...
	if (digest.qop) {
		response = malloc(
				sizeof(char)
						* (strlen(userp_quoted) + 1 + strlen(realm_quoted) + 1
								+ strlen(nonce_quoted) + 1 + strlen(uri) + 1
								+ strlen(digest.cnonce) + 1 + 8 + 1
								+ strlen(digest.qop) + 1
								+ strlen((char*) request_digest) + 1 + 256));
		sprintf(response, "Digest username=\"%s\", "
				...
				"nc=%08x, "
				"qop=%s, "
				"response=\"%s\"", userp_quoted, realm_quoted, nonce_quoted,
				uri, digest.cnonce, digest.nc, digest.qop, request_digest);
		...
		response = malloc(
				sizeof(char)
						* (strlen(userp_quoted) + 1 + strlen(realm_quoted) + 1
								+ strlen(nonce_quoted) + 1 + strlen(uri) + 1
								+ strlen((char*) request_digest) + 1 + 128));
		sprintf(response, "Digest username=\"%s\", "
				"realm=\"%s\", "
				"nonce=\"%s\", "
				"uri=\"%s\", "
	...
	if (digest.opaque) {
		...
		tmp = malloc(
				sizeof(char) * strlen(response) + strlen(opaque_quoted) + 16);
	...
	if (digest.algorithm) {
		...
		tmp = malloc(
				sizeof(char) * strlen(response) + strlen(digest.algorithm)
						+ 16);
...
static int callback_http(struct lws *wsi, enum lws_callback_reasons reason,
	...
	case LWS_CALLBACK_ESTABLISHED_CLIENT_HTTP: {
		...
		if (status == 401
				&& lws_hdr_total_length(wsi, WSI_TOKEN_HTTP_WWW_AUTHENTICATE)
						> 0) {
			www_authenticate_buffer = malloc(
					(size_t) lws_hdr_total_length(wsi,
							WSI_TOKEN_HTTP_WWW_AUTHENTICATE) + 1);
			memset(www_authenticate_buffer, 0x0,
					(size_t) lws_hdr_total_length(wsi,
							WSI_TOKEN_HTTP_WWW_AUTHENTICATE) + 1);
			lws_hdr_copy(wsi, www_authenticate_buffer, 1024,
	...
	case LWS_CALLBACK_CLIENT_APPEND_HANDSHAKE_HEADER: {
		...
		if (www_authenticate_buffer != NULL && auth_type == 2) {
			...
			output_digest(wsi, "GET", path, (char*) ba_user,
					(char*) ba_password, www_authenticate_buffer, &output,
					&output_size);
...
static const struct lws_protocols protocols[] = { { "http", callback_http, 0, 0,
		0, NULL, 0 },
...
static int system_notify_cb(lws_state_manager_t *mgr,
		lws_state_notify_link_t *link, int current, int target) {
...
int main(int argc, const char **argv) {
	lws_state_notify_link_t notifier = { { NULL, NULL, NULL }, system_notify_cb,
			"app" };
	...
	bail: if ((p = lws_cmdline_option(argc, argv, "--expected-exit")))
		expected = atoi(p);
