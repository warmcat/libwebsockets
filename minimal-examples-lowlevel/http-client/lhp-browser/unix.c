...
/*
 * Present one rendered document line at window row wy: like the EPD case
 * there is no framebuffer, the renderer's single line buffer goes
...
lhp_browser_plat_line(int wy, const uint8_t *rgb, int w)
{
	...
	if (!x11.ximg || x11.img_w != w) {
		if (x11.ximg) {
			x11.ximg->data = NULL;
			XDestroyImage(x11.ximg);
			free(x11.xdata);
		}
		x11.xdata = malloc((size_t)(w > 0 ? w : 1) * 4);
		if (!x11.xdata)
			return;
		x11.img_w = w;
		x11.ximg = XCreateImage(x11.dpy, CopyFromParent, 24, ZPixmap, 0,
					(char *)x11.xdata, (unsigned)w, 1, 32, 0);
		if (!x11.ximg)
			return;
	}
	...
	XPutImage(x11.dpy, x11.xw, x11.gc, x11.ximg, 0, 0, 0, wy,
		  (unsigned)w, 1);
	XFlush(x11.dpy);
}
...
lhp_browser_plat_clear(int wy, int wh)
{
	...
	XFillRectangle(x11.dpy, x11.xw, x11.gc, 0, wy,
		       (unsigned)win.vw, (unsigned)wh);
	XFlush(x11.dpy);
}
