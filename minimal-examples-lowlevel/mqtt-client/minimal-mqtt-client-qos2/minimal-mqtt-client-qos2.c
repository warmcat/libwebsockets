...
static const struct lws_switches switches[] = {
	...
	[LWS_SW_F]      = { "-f",              "Simulate connection drop for testing QoS2 restoration" },
...
enum {
	...
	STATE_PUBLISH_QOS2,	/* Send the message in QoS2 */
	STATE_WAIT_ACK2,	/* Wait for the real ack (PUBCOMP) */

	STATE_TEST_FINISH
};

static int interrupted, bad = 1, do_ssl, do_fault_injection;
static uint16_t simulated_saved_pkt_id;
...
static int my_rx_add(struct lws *wsi, const char *client_id, uint16_t pkt_id) {
	lwsl_user("QoS 2: Stored packet_id %d for client %s\n", pkt_id, client_id);
	simulated_saved_pkt_id = pkt_id;
	return 0;
}
static int my_rx_remove(struct lws *wsi, const char *client_id, uint16_t pkt_id) {
	lwsl_user("QoS 2: Removed packet_id %d for client %s\n", pkt_id, client_id);
	simulated_saved_pkt_id = 0;
	return 0;
}
static const lws_mqtt_qos2_state_ops_t my_qos2_ops = {
	.rx_add = my_rx_add,
	.rx_remove = my_rx_remove,
};
...
static const lws_mqtt_client_connect_param_t client_connect_param = {
	...
	.qos2_state_ops                 = &my_qos2_ops,
};

static lws_mqtt_publish_param_t pub_param;

static lws_mqtt_topic_elem_t topics[] = {
	[0] = { .name = "test/topic0", .qos = QOS2 },
	[1] = { .name = "test/topic1", .qos = QOS2 },
};
	      ...
	      void *user, void *in, size_t len)
{
	...
	case LWS_CALLBACK_MQTT_CLIENT_ESTABLISHED:
		...
		/*
		 * In a real application, you would load your saved QoS2 unacknowledged
		 * receives from your database here and inject them.
		 *
		 * For this example, we inject the simulated saved ID.
		 */
		if (simulated_saved_pkt_id) {
			lwsl_user("%s: Restoring saved QoS2 packet_id %d\n", __func__, simulated_saved_pkt_id);
			lws_mqtt_client_qos2_rx_add(wsi, simulated_saved_pkt_id);
		}
	...
	case LWS_CALLBACK_MQTT_QOS2_RX_COMPLETE:
		lwsl_user("%s: MQTT_QOS2_RX_COMPLETE packet ID %d\n", __func__, *(uint16_t *)in);
		return 0;
	...
	case LWS_CALLBACK_MQTT_CLIENT_RX:
		...
		if (do_fault_injection) {
			lwsl_user("%s: Simulating connection drop!\n", __func__);
			do_fault_injection = 0; /* only do it once */
			return -1; /* forceful drop */
		}
...
int main(int argc, const char **argv)
{
	...
	do_fault_injection = !!lws_cmdline_option(argc, argv, switches[LWS_SW_F].sw);

	if (do_ssl)
		info.options = LWS_SERVER_OPTION_DO_SSL_GLOBAL_INIT;

	lwsl_user("LWS minimal MQTT client %s [-d<verbosity>][-s][-f]\n",
