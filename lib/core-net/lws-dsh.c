...
static int
_lws_dsh_alloc_tail(lws_dsh_t *dsh, int kind, void *src, size_t size,
		    lws_dll2_t *replace);
...
evict2(struct lws_dll2 *d, void *user)
{
	...
	if (_lws_dsh_alloc_tail(dsh, 0, (void *)&obj[1], obj->size, &obj->list)) {
...
_lws_dsh_alloc_tail(lws_dsh_t *dsh, int kind, void *src, size_t size,
		    lws_dll2_t *replace)
{
	size_t asize = sizeof(lws_dsh_obj_t) + lws_dsh_align(size);
...
int
lws_dsh_alloc_tail(lws_dsh_t *dsh, int kind, void *src, size_t size)
{
	return _lws_dsh_alloc_tail(dsh, kind, src, size, NULL);
}
