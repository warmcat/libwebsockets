...
static int ssl_pm_reload_crt(SSL *ssl)
{
    ...
    if (ca_pm->x509_crt) {
        lwsl_info("ssl_pm_reload_crt: using ca_pm->x509_crt\n");
        ...
        lwsl_info("ssl_pm_reload_crt: NO CA CHAIN PROVIDED!\n");
    }
...
void SSL_set_SSL_CTX(SSL *ssl, SSL_CTX *ctx)
{
	...
	if ((ctx->verify_mode & SSL_VERIFY_POST_HANDSHAKE) > 0)
		mode = MBEDTLS_SSL_VERIFY_OPTIONAL;
	else if ((ctx->verify_mode & SSL_VERIFY_PEER) > 0)
		mode = MBEDTLS_SSL_VERIFY_REQUIRED;
