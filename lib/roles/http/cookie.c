...
lws_cookie_write_nsc(struct lws *wsi, struct lws_cookie *c)
{
	...
	if (!c->f[CE_DOMAIN]) {
		c->f[CE_HOSTONLY] = "T";
		c->l[CE_HOSTONLY] = 1;
		c->f[CE_DOMAIN] = wsi->stash->cis[CIS_ADDRESS];
		c->l[CE_DOMAIN] = strlen(c->f[CE_DOMAIN]);
	}

	if (!c->f[CE_PATH]) {
		c->f[CE_PATH] = wsi->stash->cis[CIS_PATH];
...
lws_cookie_send_cookies(struct lws *wsi, char **pp, char *end)
{
	...
	char cache_name[LWS_COOKIE_MAX_CACHE_NAME_LEN];
	size_t domain_len, path_len, size;
	struct lws_cache_ttl_lru *l1;
	...
	if (!wsi || !wsi->stash || !wsi->stash->cis[CIS_ADDRESS] ||
	    !wsi->stash->cis[CIS_PATH] || !pp || !(*pp) || !end)
		return -1;
	...
	domain = wsi->stash->cis[CIS_ADDRESS];
	path = wsi->stash->cis[CIS_PATH];
	domain_len = strlen(domain);
	path_len = strlen(path);
	p = *pp;

	lwsl_cookie("%s: domain %s len %d\n", __func__, domain, (int)domain_len);
	lwsl_cookie("%s: path %s len %d\n", __func__, path, (int)path_len);

	/* remove query string if exist */
	dl_path = memchr(path, '?', path_len);
	if (dl_path)
		path_len = (size_t)(dl_path - path);

	/* remove last slash if exist */
	if (path_len != 1 && path[path_len - 1] == '/')
		path_len--;

	if (!domain_len || !path_len)
		return -1;

	/* iterate through domain and path levels to find matching cookies */
	dl_domain = domain;
	while (dl_domain) {
		dl_domain = memchr(domain, '.', strlen(domain));
		if (!dl_domain)
			break;

		p1 = cache_name;
		memcpy(p1, domain, strlen(domain));
		p1 += strlen(domain);
		*p1 = '|';
		p1++;
	...
	if (p != *pp){
		...
		lwsl_notice("%s: c len (%d)\n", __func__,
				(int )lws_ptr_diff_size_t(p, *pp));
#if defined(LWS_COOKIE_DEBUG)
		lwsl_hexdump_notice(*pp, lws_ptr_diff_size_t(p, *pp));
#endif
		*pp = p;
	}
