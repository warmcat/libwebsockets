...
struct cb_hdr_s {
	int h_type; /* may be hash, or hmac type */
	char alg[10];
	int is_rsa:1;
};
...
cb_hdr(struct lejp_ctx *ctx, char reason)
{
	...
	case JHP_ALG:
		...
		if (!strcmp(ctx->buf, "HS256")) {
			s->h_type = LWS_GENHMAC_TYPE_SHA256;
			break;
		}
		if (!strcmp(ctx->buf, "HS384")) {
			s->h_type = LWS_GENHMAC_TYPE_SHA384;
			break;
		}
		if (!strcmp(ctx->buf, "HS512")) {
			s->h_type = LWS_GENHMAC_TYPE_SHA512;
			break;
		}
		if (!strcmp(ctx->buf, "RS256")) {
			s->h_type = LWS_GENHASH_TYPE_SHA256;
			s->is_rsa = 1;
			break;
		}
		if (!strcmp(ctx->buf, "RS384")) {
			s->h_type = LWS_GENHASH_TYPE_SHA384;
			s->is_rsa = 1;
			break;
		}
		if (!strcmp(ctx->buf, "RS512")) {
			s->h_type = LWS_GENHASH_TYPE_SHA512;
			s->is_rsa = 1;
			break;
		}
		return -1;
...
lws_jws_confirm_sig(const char *in, size_t len, struct lws_jwk *jwk)
{
	...
	if (args.is_rsa) {
		...
		if (lws_genhash_init(&hash_ctx, args.h_type))
			return -1;
		...
		h_len = lws_genhash_size(args.h_type);
		...
		n = lws_genrsa_public_verify(&rsactx, digest, args.h_type,
					     (uint8_t *)buf, m);
	...
	h_len = lws_genhmac_size(args.h_type);
	if (m < 0 || m != h_len)
		return -1;

	/* 6) compute HMAC over payload */

	if (lws_genhmac_init(&ctx, args.h_type, jwk->el.e[JWK_KEY_E].buf,
