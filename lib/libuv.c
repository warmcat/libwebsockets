...
//	struct lws_context *ctx;
	...
	if (!pt->io_loop_uv)
		return;
#if 0
	ctx = context->deprecated_context_list;
	while (ctx) {
		struct lws_context_per_thread *pt1;
		int n;
		for (n = 0; n < context->count_threads; n++) {
			pt1 = &ctx->pt[n];
			if (pt->io_loop_uv == pt1->io_loop_uv) {
				lwsl_notice("%s: deferring ctx %p, shared with deprecated context %p\n", __func__, context, ctx);
				return;
			}
		}
...
lws_libuv_stop(struct lws_context *context)
{
	...
	if (context->deprecated_context_list)
		lws_libuv_stop(context->deprecated_context_list);

	lwsl_info("%s: feels everything closed\n", __func__);
	if (context->count_wsi_allocated == 0 && !context->deprecated)
