...
void lws_esp32_restart_into_factory(void)
{
        uint32_t *p_force_factory_magic = (uint32_t *)0x50001ffc;

	lwsl_notice("%s\n", __func__);
        *p_force_factory_magic = 0xb00bcafe;
...
lws_esp_ota_get_boot_partition(void)
{
	...
	if (eih.spi_mode == 0xff ||
	    *p_force_factory_magic == 0xfaceb00b) {
		/*
...
lws_esp32_init(struct lws_context_creation_info *info)
{
	...
	lws_esp32_get_image_info(part, &i, buf, sizeof(buf));
	
	lws_esp32_romfs = (romfs_t)i.romfs;
	if (!romfs_mount_check(lws_esp32_romfs)) {
		lwsl_err("Failed to mount ROMFS at %p\n", lws_esp32_romfs);
		return NULL;
	}
