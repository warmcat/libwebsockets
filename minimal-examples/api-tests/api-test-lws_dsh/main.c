...
test1(void)
{
	...
	if (lws_dsh_alloc_tail(dsh, 0, "hello", 5)) {
		lwsl_err("%s: Failed to alloc 1\n", __func__);

		goto bail;
	}

	if (lws_dsh_alloc_tail(dsh, 1, "some other string", 17)) {
		lwsl_err("%s: Failed to alloc 2\n", __func__);

		goto bail;
	}

	if (lws_dsh_alloc_tail(dsh, 0, "hello again", 11)) {
...
test2(void)
{
	...
	if (lws_dsh_alloc_tail(dsh, 0, blob, 4000)) {
		lwsl_err("%s: Failed to alloc 1\n", __func__);

		goto bail2;
	}

	if (lws_dsh_alloc_tail(dsh2, 0, "hello", 5)) {
		lwsl_err("%s: Failed to alloc 2\n", __func__);
	...
	if (lws_dsh_alloc_tail(dsh, 0, blob, 2000)) {
		lwsl_err("%s: Failed to alloc 3\n", __func__);

		goto bail2;
	}

	if (lws_dsh_alloc_tail(dsh2, 0, "hello again", 11)) {
...
test3(void)
{
	...
	if (!lws_dsh_alloc_tail(dsh, 0, blob, 5000)) {
...
test4(void)
{
	...
	if (lws_dsh_alloc_tail(dsh, 0, "some other string", 17)) {
		lwsl_err("%s: Failed to alloc 2\n", __func__);
