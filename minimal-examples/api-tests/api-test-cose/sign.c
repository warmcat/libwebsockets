...
test_cose_sign(struct lws_context *context)
{
	...
	cps = lws_cose_sign_val_create(context, SIGTYPE_SINGLE, &set,
				       NULL, NULL, 0);
	if (!cps) {
	...
	n = lws_cose_sign_val_chunk(cps, sign1_pass_01, sizeof(sign1_pass_01),
				    NULL);
	if (n) {
	...
	o = lws_cose_sign_val_results(cps);
	if (o->count != 1)
		goto bail1;

	res = lws_container_of(o->head, lws_cose_sig_val_res_t, list);
	if (res->result)
		goto bail1;

	lws_cose_sign_val_destroy(&cps);
	...
	cps = lws_cose_sign_val_create(context, SIGTYPE_SINGLE, &set,
				       xcb, NULL, sizeof(sign1_pass_02_ext));
	if (!cps) {
		lwsl_notice("%s: sign_val_create fail\n", __func__);
		goto bail;
	}

	n = lws_cose_sign_val_chunk(cps, sign1_pass_02, sizeof(sign1_pass_02),
				    NULL);
	if (n) {
		lwsl_notice("%s: sign_val_chunk failed\n", __func__);
		goto bail1;
	...
	n = lws_cose_sign_val_chunk(cps, sign1_pass_03, sizeof(sign1_pass_03),
				    NULL);
	if (n) {
		lwsl_notice("%s: sign_val_chunk failed\n", __func__);
		goto bail1;
	...
	n = lws_cose_sign_val_chunk(cps, sign1_fail_01, sizeof(sign1_fail_01),
				    NULL);
	if (!n) {
	...
	n = lws_cose_sign_val_chunk(cps, sign1_fail_02, sizeof(sign1_fail_02),
				    NULL);
	if (n) {
		lwsl_notice("%s: sign_val_chunk failed\n", __func__);
		goto bail1;
	...
	n = lws_cose_sign_val_chunk(cps, sign1_fail_03, sizeof(sign1_fail_03),
				    NULL);
	if (n) {
		lwsl_notice("%s: sign_val_chunk failed\n", __func__);
		goto bail1;
	...
	n = lws_cose_sign_val_chunk(cps, sign1_fail_04, sizeof(sign1_fail_04),
				    NULL);
	if (n) {
		lwsl_notice("%s: sign_val_chunk failed\n", __func__);
		goto bail1;
	...
	n = lws_cose_sign_val_chunk(cps, sign1_fail_06, sizeof(sign1_fail_06),
				    NULL);
	if (n) {
		lwsl_notice("%s: sign_val_chunk failed\n", __func__);
		goto bail1;
	...
	n = lws_cose_sign_val_chunk(cps, sign1_fail_07, sizeof(sign1_fail_07),
				    NULL);
	if (n) {
		lwsl_notice("%s: sign_val_chunk failed\n", __func__);
		goto bail1;
	...
	cps = lws_cose_sign_val_create(context, SIGTYPE_MULTI, &set,
				       NULL, NULL, 0);
	if (!cps) {
		lwsl_notice("%s: sign_val_create fail\n", __func__);
		goto bail;
	...
	n = lws_cose_sign_val_chunk(cps, sign_pass_01, sizeof(sign_pass_01),
				    NULL);
	if (n) {
		lwsl_notice("%s: sign_val_chunk failed\n", __func__);
		goto bail1;
	...
	n = lws_cose_sign_val_chunk(cps, sign_pass_02, sizeof(sign_pass_02),
				    NULL);
	if (n) {
		lwsl_notice("%s: sign_val_chunk failed\n", __func__);
		goto bail1;
	...
	n = lws_cose_sign_val_chunk(cps, sign_pass_03, sizeof(sign_pass_03),
				    NULL);
	if (n) {
		lwsl_notice("%s: sign_val_chunk failed\n", __func__);
		goto bail1;
	...
	n = lws_cose_sign_val_chunk(cps, sign_fail_01, sizeof(sign_fail_01),
				    NULL);
	if (!n) {
	...
	n = lws_cose_sign_val_chunk(cps, sign_fail_02, sizeof(sign_fail_02),
				    NULL);
	if (n) {
		lwsl_notice("%s: sign_val_chunk failed\n", __func__);
		goto bail1;
	...
	n = lws_cose_sign_val_chunk(cps, sign_fail_03, sizeof(sign_fail_03),
				    NULL);
	if (n) {
		lwsl_notice("%s: sign_val_chunk failed\n", __func__);
		goto bail1;
	...
	n = lws_cose_sign_val_chunk(cps, sign_fail_04, sizeof(sign_fail_04),
				    NULL);
	if (n) {
		lwsl_notice("%s: sign_val_chunk failed\n", __func__);
		goto bail1;
	...
	n = lws_cose_sign_val_chunk(cps, sign_fail_06, sizeof(sign_fail_06),
				    NULL);
	if (n) {
		lwsl_notice("%s: sign_val_chunk failed\n", __func__);
		goto bail1;
	...
	n = lws_cose_sign_val_chunk(cps, sign_fail_07, sizeof(sign_fail_07),
				    NULL);
	if (n) {
		lwsl_notice("%s: sign_val_chunk failed\n", __func__);
		goto bail1;
	...
	cps = lws_cose_sign_val_create(context, SIGTYPE_MAC, &set,
				       NULL, NULL, 0);
	if (!cps) {
		lwsl_notice("%s: sign_val_create fail\n", __func__);
		goto bail;
	...
	n = lws_cose_sign_val_chunk(cps, sign_hmac_01, sizeof(sign_hmac_01),
				    NULL);
	if (n) {
		lwsl_notice("%s: sign_val_chunk failed\n", __func__);
		goto bail1;
	...
	n = lws_cose_sign_val_chunk(cps, sign_hmac_02, sizeof(sign_hmac_02),
				    NULL);
	if (n) {
		lwsl_notice("%s: sign_val_chunk failed\n", __func__);
		goto bail1;
	...
	n = lws_cose_sign_val_chunk(cps, sign_hmac_03, sizeof(sign_hmac_03),
				    NULL);
	if (n) {
		lwsl_notice("%s: sign_val_chunk failed\n", __func__);
		goto bail1;
	...
	n = lws_cose_sign_val_chunk(cps, sign_hmac_04, sizeof(sign_hmac_04),
				    NULL);
	if (n) {
		lwsl_notice("%s: sign_val_chunk failed\n", __func__);
		goto bail1;
	...
	n = lws_cose_sign_val_chunk(cps, sign_hmac_05, sizeof(sign_hmac_05),
				    NULL);
	if (n) {
		lwsl_notice("%s: sign_val_chunk failed\n", __func__);
		goto bail1;
	...
	cps = lws_cose_sign_val_create(context, SIGTYPE_MAC0, &set,
				       NULL, NULL, 0);
	if (!cps) {
		lwsl_notice("%s: sign_val_create fail\n", __func__);
		goto bail;
	...
	n = lws_cose_sign_val_chunk(cps, enc_hmac_01, sizeof(enc_hmac_01),
				    NULL);
	if (n) {
		lwsl_notice("%s: sign_val_chunk failed\n", __func__);
		goto bail1;
	...
	n = lws_cose_sign_val_chunk(cps, enc_hmac_02, sizeof(enc_hmac_02),
				    NULL);
	if (n) {
		lwsl_notice("%s: sign_val_chunk failed\n", __func__);
		goto bail1;
	...
	n = lws_cose_sign_val_chunk(cps, enc_hmac_03, sizeof(enc_hmac_03),
				    NULL);
	if (n) {
		lwsl_notice("%s: sign_val_chunk failed\n", __func__);
		goto bail1;
	...
	n = lws_cose_sign_val_chunk(cps, enc_hmac_04, sizeof(enc_hmac_04),
				    NULL);
	if (n) {
		lwsl_notice("%s: sign_val_chunk failed\n", __func__);
		goto bail1;
	...
	n = lws_cose_sign_val_chunk(cps, enc_hmac_05, sizeof(enc_hmac_05),
				    NULL);
	if (n) {
		lwsl_notice("%s: sign_val_chunk failed\n", __func__);
		goto bail1;
