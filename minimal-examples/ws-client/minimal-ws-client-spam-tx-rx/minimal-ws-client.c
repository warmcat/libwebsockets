         ...
         void *user, void *in, size_t len)
{
  ...
  case LWS_CALLBACK_CLIENT_WRITEABLE:
    ...
    // Schedule the timer after minimum message delay plus the random
    // number of centiseconds.
    n = message_delay + 10000*(random()%100);
  ...
  case LWS_CALLBACK_CLIENT_RECEIVE:
    ...
    lwsl_debug(
      "LWS_CALLBACK_RECEIVE: len = %lu, first = %d, final = %d, remains = %lu\n",
      len, first, final, remain);
    break;
