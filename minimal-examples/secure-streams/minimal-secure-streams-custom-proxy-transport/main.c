...
static void
lws_sss_proxy_custom_onward_bind(lws_sss_priv_t *priv, struct lws_ss_handle *h)
{
}

static void
lws_sss_proxy_custom_req_write(lws_sss_priv_t *priv)
{
...
lws_sss_proxy_custom_fault_context(lws_sss_priv_t *priv)
{
	return NULL;
}
#endif
...
static int
lws_sss_proxy_custom_write(lws_sss_priv_t *priv, uint8_t *buf, size_t len)
{
	struct lws *wsi = (struct lws *)*priv;
...
int
lws_sss_proxy_custom_init_server(struct lws_context *context, const char *bind,
					int port)
{
	/*
	 * We're just a dumb one-channel custom
	 */
...
static void
lws_sss_proxy_custom_client_up(lws_sss_priv_t *priv)
{
	struct lws *wsi = (struct lws *)*priv;
...
static const lws_sss_ops_proxy_t lws_sss_ops_proxy_custom = {
	.req_write		= lws_sss_proxy_custom_req_write,
	.write			= lws_sss_proxy_custom_write,

	.onward_bind		= lws_sss_proxy_custom_onward_bind,
	.init_server		= lws_sss_proxy_custom_init_server,
#if defined(LWS_WITH_SYS_FAULT_INJECTION)
	.fault_context		= lws_sss_proxy_custom_fault_context,
#endif
	.client_up		= lws_sss_proxy_custom_client_up
};
...
int main(int argc, const char **argv)
{
	...
	info.sss_ops_proxy = &lws_sss_ops_proxy_custom;
