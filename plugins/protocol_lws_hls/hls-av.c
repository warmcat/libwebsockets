...
static int
lws_hls_get_segment_boundaries(AVFormatContext *ctx, int video_idx, double **out_starts)
{
	double duration = (double)ctx->duration / AV_TIME_BASE;
	int est_segments = (int)(duration / HLS_SEGMENT_DUR);
	double *starts;
	int count = 1;

	if ((double)est_segments * HLS_SEGMENT_DUR < duration)
		est_segments++;

	starts = malloc(sizeof(double) * (size_t)(est_segments + 2));
	if (!starts)
		return -1;
	...
	if (video_idx >= 0) {
		int i;
		for (i = 1; i < est_segments; i++) {
			int64_t target_time = (int64_t)i * HLS_SEGMENT_DUR * AV_TIME_BASE;
			double ts = (double)lws_hls_find_keyframe_ts(ctx, video_idx, target_time) / AV_TIME_BASE;
			if (ts > starts[count - 1] + 0.5) {
				starts[count++] = ts;
			}
		}
	} else {
		int i;
		for (i = 1; i < est_segments; i++) {
			starts[count++] = (double)i * HLS_SEGMENT_DUR;
		}
	}

	if (duration > starts[count - 1] + 0.5) {
		starts[count++] = duration;
	} else {
		starts[count - 1] = duration;
	}
...
lws_hls_serve_init(struct lws *wsi, const char *media_dir, const char *filename)
{
        ...
        av_dict_set(&opts, "movflags", "empty_moov+frag_keyframe+default_base_moof+delay_moov", 0);
...
lws_hls_serve_manifest(struct lws *wsi, const char *media_dir, const char *filename)
{
	...
	num_boundaries = lws_hls_get_segment_boundaries(fmt_ctx, video_idx, &segment_starts);
	if (num_boundaries < 0) {
		avformat_close_input(&fmt_ctx);
		lws_return_http_status(wsi, HTTP_STATUS_INTERNAL_SERVER_ERROR, "OOM");
		return -1;
	}
	total_segments = num_boundaries - 1;
...
lws_hls_serve_segment(struct lws *wsi, const char *media_dir, const char *filename, int segment_idx)
{
	...
	double *segment_starts = NULL;
	int num_boundaries = lws_hls_get_segment_boundaries(in_ctx, video_idx, &segment_starts);
	int64_t start_time;
	int64_t end_time;

	if (num_boundaries < 0 || segment_idx >= num_boundaries - 1) {
		if (segment_starts)
			free(segment_starts);
		goto done;
	}

	start_time = (int64_t)(segment_starts[segment_idx] * AV_TIME_BASE);
	end_time = (int64_t)(segment_starts[segment_idx + 1] * AV_TIME_BASE);
	free(segment_starts);
