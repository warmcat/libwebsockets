		     ...
		     const lwsgw_hash *sid, char *username, int len)
{
	...
	snprintf(s, sizeof(s) - 1,
		 "select username from sessions where name = '%s';", sid->id);
		     ...
		     lwsgw_hash *sid, char *username, int exp)
{
	...
	snprintf(s, sizeof(s) - 1,
		 ...
		 "values ('%s', '%s', %u);", sid->id, sql_purify(u), exp);
...
static void
echo_read(struct uv_stream_s *server, ssize_t nread, const uv_buf_t *buf) {
	if (nread == -1) {
...
void lwsgs_email_on_connect(uv_connect_t *req, int status)
{
	struct per_vhost_data__generic_sessions *vhd = lws_container_of(req,
			struct per_vhost_data__generic_sessions, email_connect_req);
...
)
{
	...
	case LGSSMTP_IDLE:
		...
		u.username[0] = '\0';
		snprintf(s, sizeof(s) - 1,
			 "select username, creation_time, email, verified, token"
			 " from users where verified != %d limit 1;",
			 LWSGS_VERIFIED_ACCEPTED);

		if (sqlite3_exec(vhd->pdb, s, lwsgs_lookup_callback_user, &u,
				 NULL) != SQLITE_OK) {
			lwsl_err("Unable to lookup user: %s\n",
				 sqlite3_errmsg(vhd->pdb));
		...
		if (uv_ip4_addr("127.0.0.1", 25, &req_addr)) {
	...
	case LGSSMTP_CONNECTING:
		if (vhd->email_connect_started - now > 5) {
			...
			/* !!! kill the connection */
//			uv_close((uv_handle_t *) &vhd->email_connect_req,
//				 (uv_close_cb) free);
...
skip:
	...
	case LWS_CALLBACK_HTTP_BODY:
		...
		/*
		 * Whatever we're here for, we will need tha POST args
		 * processing...
		 */
		...
		if (!strcmp_end((char *)pss->onward, "/login")) {
			lwsgw_hash_bin hash_bin;
			lwsgw_hash hash;
			struct lwsgs_user u;
			unsigned char sid_rand[20];
			...
			if (formvals[FGS_REGISTER] && formvals[FGS_REGISTER][0]) {
				...
				lwsl_notice("Associating current "
					    "session %s to user %s",
...
reg_done:
		...
		if (!strcmp_end((char *)pss->onward, "/logout")) {
