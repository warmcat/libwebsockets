...
The Domains tab also carries a Server IP Inventory table below the domain list. It shows every unique address mentioned in any zonefile, grouped into one row per server: address records (A and AAAA) sharing one fully-qualified owner name are understood to be the same single machine, so its v4 and v6 addresses are listed together, along with any LOC record for that name. The DNS name links into the zonefile editor for the zonefile(s) the server was found in.

Each address tracks what kinds of records mention it. If an address is only referred to by NS records (delegation glue), it is marked "NS only", since it may not be your infrastructure; if the same address is also bound to host names in the same or another zonefile, the more specific records indicate the nameserver is your infrastructure.
