...
void *sshd_zalloc1(size_t s) { return sshd_zalloc(s); }
void *sshd_zalloc2(size_t s) { return sshd_zalloc(s); }
void *sshd_zalloc3(size_t s) { return sshd_zalloc(s); }
void *sshd_zalloc4(size_t s) { return sshd_zalloc(s); }
void *sshd_zalloc5(size_t s) { return sshd_zalloc(s); }
void *sshd_zalloc6(size_t s) { return sshd_zalloc(s); }
void *sshd_zalloc7(size_t s) { return sshd_zalloc(s); }
		    ...
		    struct lws_ssh_channel *ch)
{
	lws_start_foreach_llp(struct lws_ssh_channel **, ppch, pss->ch_list) {
		if (*ppch == ch) {
			...
			if (pss->vhd->ops && pss->vhd->ops->channel_destroy)
...
again:
		...
		case SSHS_MSG_ID:
			...
			case SSH_MSG_USERAUTH_REQUEST:
				...
				pss->ua = sshd_zalloc1(sizeof(*pss->ua));
				if (!pss->ua)
					goto bail;
		...
		case SSHS_GET_STRING_LEN_ALLOC:
			...
			pss->len = (pss->len << 8) | *p++;
                        if (++pss->ctr != 4)
                                break;
                        pss->ctr = 0;
			pss->last_alloc = sshd_zalloc2(pss->len + 1);
		...
		case SSHS_NVC_CHOPEN_TYPE:
			...
			pss->ch_temp = sshd_zalloc6(sizeof(*pss->ch_temp));
			if (!pss->ch_temp)
				return -1;
	...
	case LWS_CALLBACK_RAW_WRITEABLE:
		...
		case SSH_WT_UA_BANNER:
			...
			*pp++ = SSH_MSG_USERAUTH_BANNER;
			n = pss->vhd->ops->banner((char *)&buf[650], 150 - 1,
						  lang, sizeof(lang));
			lws_p32(pp, n);
			pp += 4;
...
bail:
	...
	case LWS_CALLBACK_CGI:
		if (pss->vhd->ops && pss->vhd->ops->child_process_io &&
	...
	case LWS_CALLBACK_CGI_TERMINATED:
		if (pss->vhd->ops && pss->vhd->ops->child_process_terminated)
