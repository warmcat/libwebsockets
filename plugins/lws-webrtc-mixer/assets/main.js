...
function updateParticipants(clients) {
    ...
    clients.forEach((c) => {
        ...
        if (participantList) {
            ...
            // Icon for everyone
            const icon = '<svg viewBox="0 0 24 24" class="icon-user-silhouette" width="20" height="20" stroke="currentColor" stroke-width="2" fill="none" stroke-linecap="round" stroke-linejoin="round"><path d="M20 21v-2a4 4 0 0 0-4-4H8a4 4 0 0 0-4 4v2"></path><circle cx="12" cy="7" r="4"></circle></svg>';
        ...
        // Overlay on video - only for joined participants and when in conference
        if (inConference && c.joined && overlayContainer && joinedIndex < 4) {
            const overlay = document.createElement('div');
            overlay.className = `name-overlay pos-${joinedIndex}`;
